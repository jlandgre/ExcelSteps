'ExcelSteps_mdlScenario cls.vb
'Version 12/2/25 minor updates
'        11/17/25 Add IsCustomMdl attr to facilitate ParseMdl option detection
Option Explicit
Public cellHome As Range
Public rngRows As Range
Public rngHeader As Range
Public wkbk As Workbook
Public wkbkParsed As Workbook '11/17/25 parsed, rows/cols wkbk
Public sht As String
Public wksht As Worksheet
Public IsCustomMdl As Boolean '11/17/25
...
ErrorExit:
    errs.RecordErr "mdlScenario.Provision", Provision
End Function
'---------------------------------------------------------------------------------------
...
'
' Created: 2/4/21 JDL  Modified: 11/4/25 add option to clear value
'
Sub SetScenModelLoc(mdl, sVar, val, Optional rngCol)
    ...
    If val <> "<clear_val>" Then
        Intersect(rngRow.EntireRow, rngCol) = val
    Else
        Intersect(rngRow.EntireRow, rngCol).ClearContents
    End If
End Sub
'---------------------------------------------------------------------------------------
...
'                          11/6/25 - Refactor for performance; use mdl.tblSteps attribute
'
Function Refresh(mdl) As Boolean
    SetErrs Refresh: If errs.IsHandle Then On Error GoTo ErrorExit
    Dim r As Object, rngRow As Variant, nm As Variant, i As Long, rowCur As Range
    ...
    If Not mdl.rngPopRows Is Nothing Then
        ...
        For Each rngRow In mdl.rngPopRows
            ...
            With r
                ...
                'Name cell/row
                If mdl.IsRngNames Then .NameRow r, mdl
                
                'Set Number Format and formula
                If Not .rngMdlCells Is Nothing Then
                    If .HasLstValidation Then .AddListValidation r
                    If .HasFormula Then _
                        If Not .WriteRowFormula(r, mdl) Then GoTo ErrorExit
                    .FormatRow r
                End If
            End With
            ...
            With mdl
                SetHeaderStyle Intersect(rngRow.EntireRow, .colrngHeaderFmt)
                SetBorders Intersect(rngRow, .colrngHeaderFmt), xlContinuous, True
            End With
        Next rngRow
        'timeEnd = Timer
        'Debug.Print "Refresh: " & mdl.mdlName & " " & Format((timeEnd - timeStart), "0.000") & " seconds"
    End If
