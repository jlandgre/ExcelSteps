...
Sub test_SetErrs_Defaults(procs)
	...
	With tst
		...
		.Assert tst, Not errs Is Nothing
		.Assert tst, errs.IsTesting = False
		.Assert tst, errs.IsShowMsgs = True

		Set errs = Nothing
		ExcelSteps.SetErrs bRet, tst.wkbkTest
		.Assert tst, bRet = True
		.Assert tst, errs.IsTesting = True
		.Assert tst, errs.IsShowMsgs = False
...
Sub test_ErrorsMeta_ResolveAndLoad(procs)
	...
	With tst
		...
		.Assert tst, meta.Init(meta, errs)
		.Assert tst, meta.ResolveCodesFromLocn(meta, errs)
		.Assert tst, meta.LoadFromLookup(meta, errs)
		.Assert tst, meta.Validate(meta, errs.Locn)

		.Assert tst, errs.iCodeReport = 101
...
Sub test_RecordErr_UserFacingCurrent(procs)
	...
	errs.RecordErr "TestProc", fRet

	With tst
		.Assert tst, fRet = False
		.Assert tst, errs.IsUserFacing = True
		.Assert tst, InStr(1, errs.ErrMsg, "User visible", vbTextCompare) > 0
		.Assert tst, InStr(1, errs.ErrMsg, "Called by", vbTextCompare) = 0
		.Update tst, procs
	End With
End Sub
'--------------------------------------------------------------------------------------
...
Sub test_RecordErr_DeveloperNestedTrace(procs)
	...
	errs.RecordErr "TestProc", fRet
	errs.RecordErr "CallerProc", fRet

	With tst
		.Assert tst, errs.IsUserFacing = False
		.Assert tst, InStr(1, errs.ErrMsg, "Developer detail", vbTextCompare) > 0
		.Assert tst, InStr(1, errs.ErrMsg, "Called by CallerProc", vbTextCompare) > 0
		.Update tst, procs
	End With
End Sub
'--------------------------------------------------------------------------------------
...
Sub test_RecordErr_BaseNotFound(procs)
	...
	errs.RecordErr "MissingProc", fRet

	With tst
		.Assert tst, InStr(1, errs.ErrMsg, "Base error code not found", vbTextCompare) > 0
		.Update tst, procs
	End With
End Sub
'--------------------------------------------------------------------------------------
