...
Option Explicit
'-------------------------------------------------------------------------------------
' Transfer a workbook's theme color scheme to another workbook
' (Not possible to set directly and loading from *.xml is cross-platform problematic)
' JDL 12/12/25
Function SetWkbkThemeColorScheme(wkbkSrc As Workbook, wkbkDest As Workbook) As Boolean
    SetErrs SetWkbkThemeColorScheme: If errs.IsHandle Then On Error GoTo ErrorExit
    Dim themePath As String
    
    'Save source workbook's theme to temp file then load to dest workbook
    themePath = Environ("TEMP") & "\tempTheme.thmx"
    wkbkSrc.Theme.ThemeColorScheme.Save themePath
    wkbkDest.Theme.ThemeColorScheme.Load themePath
...
ErrorExit:
    errs.RecordErr "SetWkbkThemeColorScheme", SetWkbkThemeColorScheme
End Function
        ...
        If Not IsEmpty(r) Then
            ...
            If Not rngRedundant Is Nothing Then
                For Each c In rngRedundant
                    errs.iCodeLocal = 2
                    errs.LookupCommentMsg c, "FlagRedundant"
                Next c
...
' Add IsError Exit and nested If 11/3/25
...
' Modified: 1/18/22 - Add IsEmptyErr check; 8/13/25; 10/30/25 update .Init args
'
    ...
    With errs
        For Each w In rng
            ...
            If .IsFail(Not rngRedundant Is Nothing, 3) Then
                ...
                .Init wkbkE:=errs.wkbkE, IsHandle:=.IsHandle
...
End Sub
'------------------------------------------------------------------------------------------------------
'Delete unneeded sheets in workbook
'Created: 12/10/25 JDL
'
Function DeleteExtraneousShts(ByVal wkbk As Workbook, ByVal sht As String) As Boolean
    SetErrs DeleteExtraneousShts: If errs.IsHandle Then On Error GoTo ErrorExit
    Dim s As Variant
    
    For Each s In wkbk.Sheets
        If s.Name <> sht Then DeleteSheet wkbk, s
    Next s
    Exit Function
    
ErrorExit:
    errs.RecordErr "DeleteExtraneousShts", DeleteExtraneousShts
End Function
...
' JDL 6/10/25; modified to check IsEmpty so still works with #NAME? error cells
'
Public Function FindEndPopulatedRange(cStart As Range, direct As xlDirection) As Range
    ...
    With cStart.Worksheet
        Select Case direct
            Case xlToRight
                c = cStart.Column
                Do While Not IsEmpty(.Cells(cStart.Row, c))
                    c = c + 1
                Loop
                Set FindEndPopulatedRange = .Cells(cStart.Row, c - 1)
            Case xlDown
                r = cStart.Row
                Do While Not IsEmpty(.Cells(r, cStart.Column))
                    r = r + 1
                Loop
...
' JDL 6/25
Public Function PickFile(Optional ByVal prompt As String = "Choose a fileÄ", _
...
' JDL 11/12/25
'
Public Function SimpleSaveAs(wkbk As Workbook, ByVal pf As String) As Boolean
    SetErrs SimpleSaveAs: If errs.IsHandle Then On Error GoTo ErrorExit
    Application.DisplayAlerts = False
...
ErrorExit:
    Application.DisplayAlerts = True
    errs.RecordErr "SimpleSaveAs", SimpleSaveAs
End Function
'-------------------------------------------------------------------------------------
'-------------------------------------------------------------------------------------
' SafeSaveAs
...
'-------------------------------------------------------------------------------------
'-------------------------------------------------------------------------------------
' SaveCopyAs with iCloud access handling
'
...
'
Sub ActiveCellColor()
    Dim lngColor As Long, r As Long, g As Long, b As Long
