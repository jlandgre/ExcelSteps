...
January 11, 2021: Significant updates to enable use as an API by calling RefreshRC() directly from VBA instead of from the ExcelSteps menu and dialog box.  Also updated such that most recipe commands work with "floating" tables that are not homed to cell A1 (header) and cell A2 (data rows).  Row/Column refresh accepts an arbitrary header range as input (rngheader argument to RefreshRC subroutine) and with arbitrary data row range (rngData argument).  The menu instructions in in Validation Tests_API.xlsm have been verified with this version.  
